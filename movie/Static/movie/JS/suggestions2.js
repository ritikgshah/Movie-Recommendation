...
document.getElementById('searchBox2').addEventListener('keyup', function(){
	...
	var cleanInput = document.getElementById('searchBox2').value.replace(/\s/g, "");
		    ...
		    for(var i = 0; i < result.d.length; i++) {
		    	...
		    	var category = result.d[i].id.slice(0,2);
		    		...
		    		if(category === "tt" && result.d[i].y) {
						...
						resultRow.setAttribute("onclick","fill2(this.value)")
...
function fill2(destinationUrl){
	document.getElementById('searchBox2').value = destinationUrl;
	document.getElementById('result2').innerHTML = "";
}